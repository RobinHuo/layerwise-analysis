...
if [ $steps -le 2 ]; then
    ...
    echo "Using checkpoint $(realpath $ckpt_dir/$model_name.pt)"
    ...
    # Extracting speaker (utterance) level representations
    rep_type=contextualized
    span=speaker
    dataset_split_arr=("dev-clean")
    for dataset_split in ${dataset_split_arr[*]}; do
...
if [ $steps -le 4 ]; then
    ...
    echo -e "\n Evaluating CCA between speaker (utterance) level representations and the corresponding one-hot embeddings"
    exp_name=cca_speaker
    span=speaker
    . scripts/get_cca_scores.sh $model_name $data_sample $exp_name $span $save_dir_pth
